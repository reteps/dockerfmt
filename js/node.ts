...
import {
  formatDockerfileContents as formatDockerfileContents_,
  FormatOptions,
} from './format.js';

const getWasm = () => {
  return fs.readFile(
    path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'format.wasm'),
  );
};
...
export const formatDockerfile = async (
  fileName: string,
  options: FormatOptions,
) => {
